...
STIME=$(($(date +%s%N)/1000000))
./4store-graph-driver/scripts/4s-query.sh  "`cat ../../tdk-codemodel/src/sources/csmr-a.sparql`" | tail -n +2 | tee results/results-a.txt
let STIME=$(($(date +%s%N)/1000000))-$STIME
echo a runtime: $STIME >> results/sum.txt
cat results/results-a.txt | wc -l >> results/results-a.txt

STIME=$(($(date +%s%N)/1000000))
./4store-graph-driver/scripts/4s-query.sh  "`cat ../../tdk-codemodel/src/sources/csmr-b.sparql`" | tail -n +2 | tee results/results-b.txt
let STIME=$(($(date +%s%N)/1000000))-$STIME
echo b runtime: $STIME >> results/sum.txt
cat results/results-b.txt | wc -l >> results/results-b.txt

STIME=$(($(date +%s%N)/1000000))
./4store-graph-driver/scripts/4s-query.sh  "`cat ../../tdk-codemodel/src/sources/csmr-c.sparql`" | tail -n +2 | tee results/results-c.txt
let STIME=$(($(date +%s%N)/1000000))-$STIME
echo c runtime: $STIME >> results/sum.txt
cat results/results-c.txt | wc -l >> results/results-c.txt

STIME=$(($(date +%s%N)/1000000))
./4store-graph-driver/scripts/4s-query.sh  "`cat ../../tdk-codemodel/src/sources/csmr-d.sparql`" | tail -n +2 | tee results/results-d.txt
let STIME=$(($(date +%s%N)/1000000))-$STIME
echo d runtime: $STIME >> results/sum.txt
cat results/results-d.txt | wc -l >> results/results-d.txt

#STIME=$(($(date +%s%N)/1000000))
#./4store-graph-driver/scripts/4s-query.sh  "`cat ../../tdk-codemodel/src/sources/csmr-e.sparql`" | tail -n +2 | tee results/results-e.txt
#let STIME=$(($(date +%s%N)/1000000))-$STIME
#echo e runtime: $STIME 
#cat results/results-e.txt | wc -l >> results/results-e.txt

#STIME=$(($(date +%s%N)/1000000))
#./4store-graph-driver/scripts/4s-query.sh  "`cat ../../tdk-codemodel/src/sources/csmr-f.sparql`" | tail -n +2 | tee results/results-f.txt
#let STIME=$(($(date +%s%N)/1000000))-$STIME
